...
<body class="antialiased p-4 md:p-8 min-h-screen">
  <div class="max-w-6xl mx-auto space-y-8">
    ...
    <!-- Header Navigation -->
    <header class="flex flex-wrap justify-between items-center pb-6 border-b border-slate-800 gap-4">
      <div>
        <div class="flex items-center gap-2 mb-1">
          <span class="inline-block w-2 h-2 rounded-full bg-emerald-500"></span>
          <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider font-mono">Telemetría Operativa</span>
        </div>
      ...
      <div class="flex items-center gap-3">
        <a href="/" class="text-xs font-medium text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 px-3.5 py-2 rounded-md border border-slate-700 transition">
          ← Volver al Terminal de Mercado
        </a>
        <button onclick="runDiagnostics()" id="btn-diag" class="text-xs font-medium text-white bg-blue-600 hover:bg-blue-500 px-4 py-2 rounded-md transition shadow-sm">
    ...
    <!-- Notification Banner -->
    <div id="diag-toast" class="hidden p-3.5 rounded-md text-xs font-mono border transition-all"></div>

    <!-- 1. Connectivity Section -->
    <section class="space-y-3">
      <h2 class="text-xs font-semibold uppercase tracking-wider text-slate-400 font-mono">Conectividad de Fuentes de Datos</h2>
    ...
    <!-- 2. Medallion Storage Telemetry (Single plane comparison) -->
    <section class="space-y-3">
      <h2 class="text-xs font-semibold uppercase tracking-wider text-slate-400 font-mono">Auditoría de Almacenamiento (Arquitectura Medallion)</h2>

      <div class="bg-slate-900 border border-slate-800 rounded-lg overflow-hidden">
        <table class="w-full text-left text-xs font-mono">
          <thead class="bg-slate-950/70 text-slate-400 border-b border-slate-800">
            <tr>
              <th class="py-3 px-4 font-semibold">Capa</th>
              <th class="py-3 px-4 font-semibold">Tecnología</th>
              <th class="py-3 px-4 font-semibold">Objetos Almacenados</th>
              <th class="py-3 px-4 font-semibold">Tamaño en Disco</th>
              <th class="py-3 px-4 font-semibold">Estado de Calidad</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-800/80">
            <tr>
              ...
              <td class="py-3 px-4 text-white" id="bronze-files-count">-- archivos (.parquet)</td>
              <td class="py-3 px-4 text-slate-300" id="bronze-size">-- KB</td>
            ...
            <tr>
              ...
              <td class="py-3 px-4 text-white" id="silver-summary">-- velas, -- posts analizados</td>
            ...
            <tr>
              ...
              <td class="py-3 px-4 text-white" id="gold-summary">-- horas consolidadas</td>
              <td class="py-3 px-4 text-slate-300" id="db-size-table">-- KB</td>
    ...
    <section class="bg-slate-900 border border-slate-800 rounded-lg p-5">
      <div class="flex flex-wrap justify-between items-center gap-4">
        <div>
          <h3 class="text-sm font-semibold text-white">Prueba de Rendimiento de Inferencia FinBERT</h3>
        ...
        <button onclick="runNlpBenchmark()" id="btn-bench" class="text-xs font-medium text-slate-200 bg-slate-800 hover:bg-slate-700 px-4 py-2 rounded-md border border-slate-700 transition">
          Ejecutar prueba de inferencia
        </button>
      </div>

      <div id="bench-box" class="hidden mt-4 pt-4 border-t border-slate-800 text-xs font-mono">
  ...
  <script>
    async function loadMetrics() {
      try {
        ...
        document.getElementById('gold-summary').textContent = `${m.gold.total_rows} registros horarios (${m.gold.symbols.join(', ')})`;
      } catch (err) {
